...
const lineAuthService = {
  getToken: async (req, cb) => {
    try {
      ...
      // 取得notify token
      const tokenResp = await fetch('https://notify-bot.line.me/oauth/token', {
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded'
        },
        body: `grant_type=authorization_code&code=${code}&client_id=${process.env.LINE_CLIENT_ID}&client_secret=${process.env.LINE_CLIENT_SECRET}&redirect_uri=${process.env.LINE_REDIRECT_URI}`,
        method: 'POST'
      })
